...
open import Neural.Smooth.Integration  -- For Fubini
...
∂-sum {zero} f x ε = 0ℝ
∂-sum {suc n} f x ε =
  (ιⁿ ε fzero) ·ℝ ∂[ f ]/∂x[ fzero ] x +ℝ
  ∂-sum (f ∘ (λ rest → (x fzero) ∷ rest)) (λ i → x (fsuc i)) (λ i → ιⁿ ε (fsuc i) , {!!})
...
postulate
  chain-rule-multivariable : (f : ℝⁿ 3 → ℝ) (u v w : ℝⁿ 3 → ℝ) (x : ℝⁿ 3) →
    let h = λ p → f (λ i → if i == fzero then u p
                           else if i == fsuc fzero then v p
                           else w p)
    in ∂[ h ]/∂x[ fzero ] x ≡
...
E-coeff σ (u , v) =
  let (x , y , z) = σ (u , v)
      xᵤ = ∂[ (λ p → fst (σ p)) ]/∂x[ fzero ] (λ { fzero → u ; (fsuc fzero) → v })
      yᵤ = ∂[ (λ p → fst (snd (σ p))) ]/∂x[ fzero ] (λ { fzero → u ; (fsuc fzero) → v })
      zᵤ = ∂[ (λ p → snd (snd (σ p))) ]/∂x[ fzero ] (λ { fzero → u ; (fsuc fzero) → v })
  in (xᵤ ²) +ℝ (yᵤ ²) +ℝ (zᵤ ²)
...
heat-equation T k ℓ =
  ∀ (x t : ℝ) →
    k ·ℝ ∂[ (λ p → T (fst p , snd p)) ]/∂x[ fsuc fzero ] (λ { fzero → x ; (fsuc fzero) → t }) ≡
    ℓ ·ℝ ∂[ (λ p → ∂[ (λ q → T (fst q , snd q)) ]/∂x[ fzero ] (λ { fzero → fst p ; (fsuc fzero) → snd p })) ]/∂x[ fzero ] (λ { fzero → x ; (fsuc fzero) → t })
...
euler-continuity u v w =
  ∀ (x y z : ℝ) →
    let p = (x , y , z)
        uₓ = ∂[ (λ q → u q) ]/∂x[ fzero ] (λ { fzero → x ; (fsuc fzero) → y ; (fsuc (fsuc fzero)) → z })
        vᵧ = ∂[ (λ q → v q) ]/∂x[ fsuc fzero ] (λ { fzero → x ; (fsuc fzero) → y ; (fsuc (fsuc fzero)) → z })
        wᵧ = ∂[ (λ q → w q) ]/∂x[ fsuc (fsuc fzero) ] (λ { fzero → x ; (fsuc fzero) → y ; (fsuc (fsuc fzero)) → z })
    in uₓ +ℝ vᵧ +ℝ wᵧ ≡ 0ℝ
...
euler-perfect-fluid u v w p =
  ∀ (x y z t : ℝ) →
    let u⁺ = u-acceleration u v w (x , y , z , t)
        pₓ = ∂[ p ]/∂x[ fzero ] (λ { fzero → x ; (fsuc fzero) → y ; (fsuc (fsuc fzero)) → z ; (fsuc (fsuc (fsuc fzero))) → t })
    in (-ℝ pₓ) ≡ u⁺
...
wave-equation-rigorous u c =
  -- Assuming u has small amplitude
  SmallAmplitude (λ x → u (x , {!!})) →
  ∀ (x t : ℝ) →
    let uₜₜ = {!!}  -- Second partial in t
        uₓₓ = {!!}  -- Second partial in x
    in uₜₜ ≡ (c ²) ·ℝ uₓₓ
...
Δ* : Type
Δ* = Σ (ℝ × ℝ) (λ (ε , η) → is-n-microvector (λ { fzero → ε ; (fsuc fzero) → η }))
...
postulate
  cauchy-riemann : (f : ℝ × ℝ → ℝ × ℝ) →
    ...
    in is-analytic f ≃
       (∀ (x y : ℝ) →
         (∂[ u ]/∂x[ fzero ] (λ { fzero → x ; (fsuc fzero) → y }) ≡
          ∂[ v ]/∂x[ fsuc fzero ] (λ { fzero → x ; (fsuc fzero) → y })) ×
         (∂[ v ]/∂x[ fzero ] (λ { fzero → x ; (fsuc fzero) → y }) ≡
          -ℝ ∂[ u ]/∂x[ fsuc fzero ] (λ { fzero → x ; (fsuc fzero) → y })))
