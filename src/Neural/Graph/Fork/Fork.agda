...
module Neural.Graph.Fork.Fork where

open import Neural.Graph.Base
open import Neural.Graph.Oriented
open import Neural.Graph.Path hiding (_++_; ++-idr; ++-assoc)  -- Hide Path-in operators, we define them for ForkPath
...
open import Data.List hiding (_++_; ++-idr; ++-assoc)  -- Hide list operators to avoid clash with ForkPath operators
...
module ForkConstruction {o ℓ} (G : Graph o ℓ)
  ...
  **Mathematical justification**:
  ...
  For ForkPath to be used as morphisms in a category, we need:
  - Composition: _++_
  - Right identity law: ++-idr
  - Associativity law: ++-assoc

  These follow the same structure as Path-in from 1Lab, but work with our HIT.
  The path-unique cases are automatic since ForkPath is propositional.
  -}

  infixr 30 _++_

  _++_ : ∀ {v w z} → ForkPath v w → ForkPath w z → ForkPath v z
  nil ++ q = q
  cons e p ++ q = cons e (p ++ q)
  path-unique p p' i ++ q = path-unique (p ++ q) (p' ++ q) i
  p ++ path-unique q q' i = path-unique (p ++ q) (p ++ q') i

  -- Right identity: p ++ nil ≡ p
  ++-idr : ∀ {v w} (p : ForkPath v w) → p ++ nil ≡ p
  ++-idr nil = refl
  ++-idr (cons e p) = ap (cons e) (++-idr p)
  ++-idr (path-unique p q i) = ap₂ (λ x y → path-unique x y i) (++-idr p) (++-idr q)

  -- Associativity: (p ++ q) ++ r ≡ p ++ (q ++ r)
  ++-assoc : ∀ {v w x y} (p : ForkPath v w) (q : ForkPath w x) (r : ForkPath x y)
           → (p ++ q) ++ r ≡ p ++ (q ++ r)
  ++-assoc nil q r = refl
  ++-assoc (cons e p) q r = ap (cons e) (++-assoc p q r)
  ++-assoc (path-unique p p' i) q r = ap₂ (λ x y → path-unique x y i) (++-assoc p q r) (++-assoc p' q r)
