...
🔧 **Module parameters** (real analysis axioms):
...
open import Order.Cat using (is-thin; poset→category)
...
open import Neural.Information
  using (ℝ; _+ℝ_; _*ℝ_; _/ℝ_; _≤ℝ_; _≥ℝ_; zeroℝ; oneℝ; logℝ; -ℝ_; sumℝ; ≤ℝ-refl; ≤ℝ-trans)
open import Neural.Code.Probabilities
  using (PfObject; PfMorphism; Pf; FinitePointedSet; underlying-set;
         ProbabilityMeasure; FiberwiseMeasure; basepoint)
...
-}
record RealAnalysisStructure : Type₁ where
  field
    ...
    {-| Helper for log-term: the actual computation p * log p ≤ 0 for probabilities -}
    ...
    ≥ℝ-prop : {x y : ℝ} → is-prop (x ≥ℝ y)

    {-| Sum preserves non-negativity -}
    sum-nonneg : {n : Nat} → (f : Fin n → ℝ) →
      ((i : Fin n) → zeroℝ ≤ℝ f i) → zeroℝ ≤ℝ sumℝ {n} f

    {-| Probabilities are bounded by 1 (note: prob-nonneg comes from ProbabilityMeasure.prob-nonneg field) -}
    prob-le-one : {X : FinitePointedSet} → (P : ProbabilityMeasure X) → (x : underlying-set X) → P .prob x ≤ℝ oneℝ

    {-| Negation distributes over sum -}
    neg-sum : {n : Nat} → (f : Fin n → ℝ) →
      -ℝ (sumℝ {n} f) ≡ sumℝ {n} (λ i → -ℝ (f i))

    {-| Negation flips order -}
    ...
    {-| Product of non-negative reals is non-negative -}
    product-nonneg : {x y : ℝ} → x ≥ℝ zeroℝ → y ≥ℝ zeroℝ → (x *ℝ y) ≥ℝ zeroℝ

    {-| Fiberwise measures are non-negative -}
    fiberwise-nonneg :
      {X Y : FinitePointedSet} →
      (f : underlying-set X → underlying-set Y) →
      (Λ : FiberwiseMeasure f) →
      (y : underlying-set Y) → (x : underlying-set X) →
      zeroℝ ≤ℝ Λ y x

{-|
## Order Conversion Helpers

Since ≥ℝ is defined as (x ≥ℝ y = y ≤ℝ x), conversions are trivial.
-}
...
{-|
  ...
  **Convention**: 0 · log 0 = 0 (by continuity)
  ...
  Uses the is-zeroℝ test from the RealAnalysisStructure parameter.
  -}
  log-term : ℝ → ℝ
  log-term p with is-zeroℝ p
  ... | true  = zeroℝ
  ... | false = p *ℝ logℝ p
  ...
  {-|
  Helper: log-term is non-positive for probabilities
  -}
  log-term-nonpos-lem : (p : ℝ) → zeroℝ ≤ℝ p → p ≤ℝ oneℝ → (log-term p) ≤ℝ zeroℝ
  log-term-nonpos-lem p p-nonneg p-le-one with is-zeroℝ p
  ... | true  = ≤ℝ-refl  -- 0 ≤ 0
  ... | false = log-term-nonpos p p-nonneg p-le-one  -- p * log p ≤ 0
  ...
  shannon-entropy :
    ...
    ℝ
  shannon-entropy {X} P =
    -ℝ (sumℝ {suc X} (λ x → log-term (P .prob x)))
  ...
  **Proof**: Uses log-term-nonneg axiom from RealAnalysisStructure,
  which states that -p log p ≥ 0 for probabilities 0 ≤ p ≤ 1.
  ...
  shannon-entropy-nonneg {X} P =
    -- shannon-entropy P = -ℝ (sumℝ (λ x → log-term (P .prob x)))
    -- By neg-sum: = sumℝ (λ x → -ℝ (log-term (P .prob x)))
    -- Each log-term p ≤ 0, so -ℝ (log-term p) ≥ 0 by neg-≤
    -- Since log-term p ≤ℝ zeroℝ, neg-≤ gives (-ℝ zeroℝ) ≤ℝ (-ℝ log-term p)
    -- By neg-zero: -ℝ zeroℝ ≡ zeroℝ, so zeroℝ ≤ℝ (-ℝ log-term p)
    -- By sum-nonneg: sum ≥ 0
    -- Transport along neg-sum equality
    transport (λ i → zeroℝ ≤ℝ sym (neg-sum (λ x → log-term (P .prob x))) i)
              (sum-nonneg (λ x → -ℝ (log-term (P .prob x)))
                          (λ i → transport (λ j → neg-zero j ≤ℝ (-ℝ log-term (P .prob i)))
                                           (neg-≤ (log-term-nonpos-lem (P .prob i)
                                                                        (P .prob-nonneg i)
                                                                        (prob-le-one P i)))))
  ...
  {-|
  Weighted conditional entropy is non-negative
  -}
  weighted-conditional-entropy-nonneg :
    {X Y : FinitePointedSet} →
    (P : ProbabilityMeasure Y) →
    (Q : (y : underlying-set Y) → ProbabilityMeasure X) →
    zeroℝ ≤ℝ weighted-conditional-entropy P Q
  weighted-conditional-entropy-nonneg {X} {Y} P Q =
    sum-nonneg (λ y → (P .prob y) *ℝ shannon-entropy (Q y))
               (λ y → ≥ℝ-to-≤ℝ (product-nonneg (≤ℝ-to-≥ℝ (P .prob-nonneg y))
                                                (≤ℝ-to-≥ℝ (shannon-entropy-nonneg (Q y)))))
  ...
  **Proof sketch**: Uses log-product, sum-distrib, and sum-factor axioms
  ...
  -- Proof uses log-product, sum-distrib, sum-factor from RealAnalysisStructure
  -- to expand S(P') = -Σ_{x,y} P(y)Q(x|y) log(P(y)Q(x|y)) and rearrange
  ...
  **Equivalence with order structures**:
  ...
  Constructed from the poset structure using 1Lab's poset→category.
  -}
  
  -- First, build the poset structure for (ℝ, ≥)
  ℝ-≥-poset : Poset lzero lzero
  ℝ-≥-poset = record
    { Ob = ℝ
    ; _≤_ = _≥ℝ_
    ; ≤-thin = ≥ℝ-prop
  ...
  ℝ-thin-category = poset→category ℝ-≥-poset
  ...
  **Pf,s**: Subcategory of Pf where:
  ...
  is-surjection :
    ...
    Type
  is-surjection {X} {Y} f =
    (y : underlying-set Y) → Σ[ x ∈ underlying-set X ] (f x ≡ y)
  
  {-|
  Fiberwise probability predicate (constructive definition)
  
  λ_y is a probability measure on fiber f⁻¹(y) if:
  1. All λ_y(x) are non-negative
  2. For each y, Σ_{x : f x = y} λ_y(x) = 1
  ...
  is-fiberwise-probability :
    ...
    (Λ : FiberwiseMeasure f) →
    Type
  is-fiberwise-probability {X} {Y} f Λ =
    {- For each y in Y, the sum of Λ y x over the fiber f⁻¹(y) equals 1 -}
    (y : underlying-set Y) →
      sumℝ {suc X} (λ x →
        if is-zeroℝ (Λ y x) then zeroℝ else Λ y x) ≡ oneℝ
  ...
  **Functoriality**:
  ...
  -}
  {-|
  Helper: Convert fiberwise measure at y into a ProbabilityMeasure

  For a PfSurjectiveMorphism with fiberwise probability Λ,
  Λ(y, -) : X → ℝ is a probability distribution on X for each y.
  -}
  fiberwise-to-prob :
    {X Y : FinitePointedSet} →
    (f : underlying-set X → underlying-set Y) →
    ...
    (y : underlying-set Y) →
    ProbabilityMeasure X
  ProbabilityMeasure.prob (fiberwise-to-prob {X} {Y} f Λ y) x = Λ y x
  ProbabilityMeasure.prob-nonneg (fiberwise-to-prob {X} {Y} f Λ y) x = fiberwise-nonneg {X} {Y} f Λ y x
  ProbabilityMeasure.prob-normalized (fiberwise-to-prob {X} {Y} f Λ y) = tt
  ProbabilityMeasure.basepoint-positive (fiberwise-to-prob {X} {Y} f Λ y) = fiberwise-nonneg {X} {Y} f Λ y (basepoint X)
  ...
  information-loss :
    ...
    ℝ
  information-loss {XP} {YP} ϕ =
    let P = XP .snd
        Q = YP .snd
        f = ϕ .underlying .func
        Λ = ϕ .underlying .fiberwise
        -- For each y, extract the conditional distribution Λ|y
    in weighted-conditional-entropy Q (λ y → fiberwise-to-prob f Λ y)
  
  {-|
  For morphism (f, Λ) : (X,P) → (Y,Q), we have S(P) ≥ S(Q)
  
  **Proof**: By extensivity lemma, S(P) = S(Q) + Σ_y Q(y) S(Λ|y).
  Since S(Λ|y) ≥ 0 for all y (by shannon-entropy-nonneg),
  all terms in the sum are non-negative, so S(P) ≥ S(Q).
  -}
  postulate
    ...
    -- Proof outline: By extensivity shannon-entropy (XP .snd) ≡ shannon-entropy (YP .snd) +ℝ information-loss ϕ
    -- Since information-loss ϕ ≥ℝ zeroℝ (sum of non-negative conditional entropies),
    -- we have shannon-entropy (XP .snd) ≥ℝ shannon-entropy (YP .snd)
    -- Requires: addition-preserves-≥ℝ axiom in RealAnalysisStructure
  
  {-|
  Information loss formula: definitional equality

  By definition, information-loss is the weighted conditional entropy.
  -}
  information-loss-formula :
    {XP YP : PfObject} →
    (ϕ : PfSurjectiveMorphism XP YP) →
    information-loss ϕ ≡
      weighted-conditional-entropy (YP .snd)
        (λ y → fiberwise-to-prob (ϕ .underlying .func) (ϕ .underlying .fiberwise) y)
  information-loss-formula {XP} {YP} ϕ = refl
